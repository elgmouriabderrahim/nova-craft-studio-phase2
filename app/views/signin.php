
<div class="w-full pt-[80px] min-h-[calc(100vh-70px)] flex flex-col items-center gap-8 lg:flex-row-reverse lg:justify-evenly">

  <div class="flex flex-col gap-4 items-center lg:gap-16">
    <h1 class="text-2xl">Nova Craft STUDIO</h1>
    <p>Sign in to continue</p>
  </div>

  <form method="POST" class="flex flex-col gap-8 items-center bg-[#C9C9C9] rounded-lg w-[90%] max-w-[500px] py-8 h-auto">
    <?php if (isset($errors['info'])): ?>
      <span class="text-red-500 self-center"><?= $errors['info'] ?></span>
    <?php endif; ?>
    <div class="w-[90%] flex flex-col">
      <label for="email" class="text-black">Email</label>
      <input
        type="email"
        id="email"
        name="email"
        placeholder="email"
        value="<?= htmlspecialchars($email ?? '') ?>"
        class="rounded-md px-4 py-2 w-full text-black outline-none"
      >
      <?php if (isset($errors['email'])): ?>
        <span class="text-red-500"><?= $errors['email'] ?></span>
      <?php endif; ?>
    </div>

    <div class="w-[90%] flex flex-col">
      <label for="password" class="text-black">Password</label>
      <input
        type="password"
        id="password"
        name="password"
        placeholder="password"
        class="rounded-md px-4 py-2 w-full text-black outline-none"
      >
      <?php if (isset($errors['password'])): ?>
        <span class="text-red-500"><?= $errors['password'] ?></span>
      <?php endif; ?>
    </div>

    <button class="w-[90%] rounded-md h-8 text-white bg-[#2C2C2C]">
      Sign In
    </button>

    <a class="w-[90%] underline text-black" href="#">
      Forgot your password?
    </a>

    <a class="w-[90%] underline text-black" href="/signup">
      Sign Up?
    </a>

  </form>
</div>
